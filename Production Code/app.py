...
def homepage():
    ...
    '''
    random_ic_link = request.url_root + 'ic_name=' + datasource.random_ice_cream().name
...
def sort_result_page():
    '''
        ...
        PARAMETERS: None
        RETURNS: page rendered by the html file 'search_result.html' 
    '''
    input = request.args["input"]
    ic_list = datasource.sort_by_ratings(input)
    random_ic_link = request.url_root + 'ic_name=' + datasource.random_ice_cream().name
    return render_template('search_result.html', header1="Top " + input + " Ice Creams" , ic_list=ic_list,\
...
def search_result_page(feature, input):
    ...
    if feature == 'brand_search':
        ic_list = datasource.search_by_brands(input)
        if input == 'bj':
            header1 = 'Ben & Jerry\'s'
        elif input == 'hd':
            header1 = 'Häagen-Dazs'
        ...
        ic_list = datasource.search_by_ratings(input)
        header1 = input + '-Star Ice Creams'

    random_ic_link = request.url_root + 'ic_name=' + datasource.random_ice_cream().name
    return render_template('search_result.html', header1=header1, ic_list=ic_list,\
        random_ic_link=random_ic_link)
...
def keyword_search_result_page():
    ...
    ic_list = datasource.search_by_keyword(input)
    random_ic_link = request.url_root + 'ic_name=' + datasource.random_ice_cream().name
    return render_template('search_result.html', header1='Keyword: '+input, ic_list=ic_list,\
        random_ic_link=random_ic_link)
...
def ice_cream_page(name):
    ...
    '''
    ic_list = datasource.search_by_name(name)
    if ic_list == []:
